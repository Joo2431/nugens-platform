...
function resolveVideoResource(url) {
  if (!url) return Promise.resolve(null);
  const raw = String(url).trim();
  const cloudflareId = getCloudflareStreamId(raw);
  if (cloudflareId) {
    return Promise.resolve({ type: "cloudflare", src: `https://iframe.videodelivery.net/${cloudflareId}` });
  }
  ...
  return Promise.resolve({ type: "video", src: publicUrl });
}
...
export default function CoursePlayer({ profile }) {
  ...
  useEffect(() => {
    ...
    setVideoType("video");
    setVideoError(false);
    resolveVideoResource(activeLesson.video_url).then(resource => {
      setVideoSrc(resource?.src || null);
      setVideoType(resource?.type || "video");
    });
  }, [activeLesson?.id]);
              ...
              <div style={{ background:"#111", borderRadius:12, overflow:"hidden",
                ...
                {videoError ? (
                  <div style={{ width:"100%", height:"100%", display:"flex",
                    ...
                    <button onClick={()=>{ setVideoError(false); resolveVideoResource(activeLesson.video_url).then(resource => {
                        setVideoSrc(resource?.src || null);
                        setVideoType(resource?.type || "video");
                      }); }}
