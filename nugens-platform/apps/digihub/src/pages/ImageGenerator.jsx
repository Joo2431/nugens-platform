...
export default function ImageGenerator({ profile }) {
  ...
  // Shows whether Brand Voice will be applied — the backend auto-injects it
  // into every generation now, so this is purely a "here's what's active"
  // indicator, not something the user has to remember to attach.
  useEffect(() => {
    const token_ = async () => (await supabase.auth.getSession()).data?.session?.access_token;
    (async () => {
      try {
        const token = await token_();
        const res = await fetch(`${API}/api/digihub/brand-voice`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
        const data = await res.json();
        if (data?.brandVoice?.brand_name) setBrandVoice(data.brandVoice);
      } catch {}
    })();
  }, []);
  ...
  const generate = async () => {
    ...
    const fullPrompt = `${style} style. For ${purpose}. ${prompt}. High quality, professional design.`;

    // Helper: try Pollinations (free, no key needed)
    const tryPollinations = async () => {
      const encoded = encodeURIComponent(fullPrompt);
      const dims = size.value === "1792x1024" ? "width=1792&height=1024"
                 : size.value === "1024x1792" ? "width=1024&height=1792"
                 : "width=1024&height=1024";
      const url = `https://image.pollinations.ai/prompt/${encoded}?${dims}&model=flux&nologo=true&enhance=true&seed=${Math.floor(Math.random()*99999)}`;
      // Verify the URL actually resolves
      const check = await fetch(url, { method:"HEAD", signal:AbortSignal.timeout(12000) });
    ...
    try {
      ...
      try {
        ...
        const data = await res.json();
        if (data.limitReached) { quotaBlocked = true; throw new Error(data.error); }
        if (!res.ok||data.error) throw new Error(data.error||data.details||`Error ${res.status}`);
        if (!data.url) throw new Error("No image URL");
        imageUrl = data.url;
      } catch (dalleErr) {
        if (dalleErr.name === "AbortError") throw dalleErr;
        // Real quota enforcement: a monthly limit hit should never silently
        // fall through to the free Pollinations fallback — that would let
        // anyone bypass their plan's limit for free. Only fall back for
        // genuine service failures.
        if (quotaBlocked) throw dalleErr;
        console.warn("DALL-E failed, falling back to Pollinations:", dalleErr.message);
        imageUrl = await tryPollinations();
      }

      setImgUrl(imageUrl);
      if (profile?.id){
        const {data:saved} = await supabase.from("dh_generated_images")
          .insert({user_id:profile.id,prompt:prompt.trim(),prompt_short:prompt.trim().slice(0,80),style,purpose,size_label:size.label,image_url:imageUrl})
          .select().single();
  ...
  return (
    <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",padding:"28px 32px",background:LIGHT,minHeight:"100vh"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); .img-thumb{transition:all 0.15s;cursor:pointer} .img-thumb:hover{transform:scale(1.04);box-shadow:0 4px 16px rgba(0,0,0,0.12)} @keyframes bounce{0%,80%,100%{transform:translateY(0)}40%{transform:translateY(-8px)}} textarea:focus,input:focus,select:focus{border-color:${PINK}!important;outline:none} .dh-img-grid{display:grid;grid-template-columns:1fr 1fr;gap:24px;max-width:1100px} @media(max-width:800px){.dh-img-grid{grid-template-columns:1fr!important}}`}</style>
      ...
      <div style={{marginBottom:28}}>
        ...
        {brandVoice ? (
          <div style={{marginTop:10,display:"inline-flex",alignItems:"center",gap:7,fontSize:12,fontWeight:600,color:"#16a34a",background:"#16a34a12",border:"1px solid #16a34a30",borderRadius:8,padding:"5px 12px"}}>
            ✓ Using your Brand Voice — {brandVoice.brand_name} ({brandVoice.tone || "no tone set"})
          </div>
        ) : (
          <a href="/brand" style={{marginTop:10,display:"inline-flex",alignItems:"center",gap:7,fontSize:12,fontWeight:600,color:MUTED,background:"#f8f9fb",border:`1px solid ${BORDER}`,borderRadius:8,padding:"5px 12px",textDecoration:"none"}}>
            Set up Brand Voice so every image matches your brand →
          </a>
        )}
      </div>

      <div className="dh-img-grid">
        {/* Controls */}
