  ...
  return (
    <div style={{ fontFamily:"'Plus Jakarta Sans',sans-serif", padding:"32px 28px 80px", background:"#f8f9fb", minHeight:"100vh" }}>
      <style>{`
        ...
        .tool-card { background:#ffffff; border:1px solid ${B}; border-radius:12px; padding:22px; cursor:pointer; transition:all 0.18s; }
        .tool-card:hover { border-color:#243040; transform:translateY(-1px); }
        .tool-card.sel { border-color:${BLUE}60; background:#f0f9ff; }
        .dh-input { width:100%; padding:10px 14px; background:#f8f9fb; border:1px solid ${B}; border-radius:8px; color:#0a0a0a; font-size:13.5px; font-family:'Plus Jakarta Sans',sans-serif; outline:none; resize:vertical; }
      ...
      <div style={{ marginBottom:32 }}>
        <h1 style={{ fontWeight:800, fontSize:"clamp(20px,2.5vw,26px)", letterSpacing:"-0.03em", color:"#0a0a0a", marginBottom:4 }}>Brand Tools</h1>
        <p style={{ fontSize:13.5, color:"#6b7280" }}>AI-powered tools to create, design, and write for your brand.</p>
      </div>
      ...
      {/* Generator panel */}
      {active && (
        <div style={{ background:"#ffffff", border:`1px solid ${B}`, borderRadius:14, padding:28 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:24 }}>
            <span style={{ fontSize:22, color:active.color }}>{active.icon}</span>
            <h2 style={{ fontWeight:700, fontSize:16, color:"#0a0a0a", letterSpacing:"-0.02em" }}>{active.title}</h2>
          </div>

          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:14 }}>
            <div>
              <label style={{ fontSize:12, fontWeight:600, color:"#6b7280", display:"block", marginBottom:6 }}>Brand Name</label>
              <input className="dh-input" value={brandName} onChange={e=>setBrandName(e.target.value)} placeholder="e.g. Zara Fitness" />
            </div>
            <div>
              <label style={{ fontSize:12, fontWeight:600, color:"#6b7280", display:"block", marginBottom:6 }}>Tagline</label>
              <input className="dh-input" value={tagline} onChange={e=>setTagline(e.target.value)} placeholder="e.g. Stronger every day" />
            </div>
            <div>
              <label style={{ fontSize:12, fontWeight:600, color:"#6b7280", display:"block", marginBottom:6 }}>Brand Color</label>
            ...
            <div>
              <label style={{ fontSize:12, fontWeight:600, color:"#6b7280", display:"block", marginBottom:6 }}>Format/Size</label>
          ...
          <div style={{ marginBottom:18 }}>
            <label style={{ fontSize:12, fontWeight:600, color:"#6b7280", display:"block", marginBottom:6 }}>Additional brief</label>
          ...
          {result && (
            <div style={{ marginTop:24, background:"#f8f9fb", border:`1px solid ${BLUE}30`, borderRadius:10, padding:20 }}>
              ...
              <pre style={{ fontSize:13.5, color:"#0a0a0a", lineHeight:1.75, whiteSpace:"pre-wrap", fontFamily:"'Plus Jakarta Sans',sans-serif", margin:0 }}>{result}</pre>
            </div>
          )}
        </div>
      )}
