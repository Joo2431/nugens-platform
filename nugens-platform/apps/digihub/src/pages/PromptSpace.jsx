...
export default function PromptSpace({ profile }) {
  ...
  const generatePrompt = async () => {
    ...
    try {
      ...
      // Real quota check — previously "10/50/unlimited per month" on the
      // pricing page was unenforced; this actually counts and blocks now.
      const usageRes = await fetch(`${API}/api/digihub/usage/check`, {
        method:"POST",
        headers:{"Content-Type":"application/json",...(token?{Authorization:`Bearer ${token}`}:{})},
        body:JSON.stringify({ feature:"prompt_space" }),
      });
      const usageData = await usageRes.json();
      if (!usageRes.ok) {
        setError(usageData.error || "Monthly limit reached. Upgrade for more.");
        setLoading(false);
        return;
      }
  ...
  return (
    <div style={{minHeight:"100vh",background:BG,padding:"32px 36px",fontFamily:"'Plus Jakarta Sans',sans-serif",color:TEXT}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); select:focus,input:focus{border-color:${PINK}!important;outline:none} .dh-ps-layout{display:grid;grid-template-columns:340px 1fr;gap:24px;max-width:1100px} @media(max-width:800px){.dh-ps-layout{grid-template-columns:1fr!important}}`}</style>
      ...
      <div className="dh-ps-layout" style={{alignItems:"start"}}>
