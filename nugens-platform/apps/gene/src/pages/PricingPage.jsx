...
const PINK   = "#e8185d";
const B      = "#1e1e1e";
const MUTED  = "#9ca3af"; // was #555 in multiple spots — failed contrast against the #09090a/#111 dark background
const FAINT  = "#6b7280"; // for "locked" / unavailable features — still reads as disabled but is actually legible
const API    = import.meta.env.VITE_GEN_E_API_URL || "https://nugens-platform-production.up.railway.app";

const INDIVIDUAL_PLANS = [
  {
    id:"free", name:"Starter", price:"₹0", period:"free forever", badge:null, color:MUTED,
  ...
  {
    // FIXED: was ₹99/mo — priced below HyperX Premium and DigiHub Monthly
    // (both ₹299/mo) despite Gen-E Pro doing strictly more for the money.
    // Raised to match the platform's own established entry-paid-tier
    // floor, and yearly re-priced to the standard "2 months free" (10x
    // monthly) instead of the previous 41% discount.
    id:"monthly", name:"Pro Monthly", price:"₹299", period:"per month", badge:"Most Popular", color:PINK,
    razorpay:"monthly", amount:29900,
  ...
  {
    id:"yearly", name:"Pro Yearly", price:"₹2,990", period:"per year · 2 months free", badge:"Best Value", color:"#7c3aed",
    razorpay:"yearly", amount:299000,
          ...
          <div style={{display:"flex",background:"#111",border:`1px solid ${B}`,borderRadius:10,padding:3}}>
            {[["individual","👤 Individual"],["business","🏢 Business"]].map(([id,lbl])=>(
              <button key={id} onClick={()=>setTab(id)}
                style={{padding:"8px 24px",borderRadius:8,border:"none",background:tab===id?PINK:"transparent",color:tab===id?"#fff":MUTED,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",transition:"all 0.14s"}}>
                {lbl}
              </button>
            ))}
          </div>
        ...
        {plans.map(p=>{
          ...
          return (
            <div key={p.id} style={{flex:1,minWidth:260,maxWidth:310,background:"#111",border:`1.5px solid ${p.badge?p.color:B}`,borderRadius:16,padding:"28px 24px",position:"relative"}}>
              ...
              <div style={{fontSize:12,color:MUTED,marginBottom:22}}>{p.period}</div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:24}}>
                ...
                {p.locked.map(f=>(
                  <div key={f} style={{display:"flex",alignItems:"flex-start",gap:7,opacity:0.6}}>
                    <span style={{color:FAINT,fontSize:12,flexShrink:0,marginTop:1}}>✗</span>
                    <span style={{fontSize:12.5,color:FAINT}}>{f}</span>
                  </div>
                ))}
              </div>
              ...
              <button
                ...
                style={{width:"100%",padding:"11px 0",borderRadius:10,border:"none",
                  ...
                  color:isCurrent?MUTED:p.badge?"#fff":"#0a0a0a",
