...
/** Map a PLATFORM_LINKS url to the app key used by hasAppAccess() */
function appKeyFor(url) {
  ...
  return "nugens"; // Nugens Dashboard — always accessible
}

const INDIVIDUAL_NAV = [
  { path:"/",         query:"",          icon:"⊞", label:"Dashboard"       },
  { path:"/chat",     query:"",          icon:"◎", label:"Career AI"       },
  { path:"/resumes",  query:"",          icon:"▤", label:"Resume Vault"    },
  { path:"/jobs",     query:"",          icon:"◑", label:"Job Tracker"     },
  { path:"/skill-gap",query:"",          icon:"◈", label:"Skill Gap"       },
  { path:"/simulate", query:"",          icon:"⬡", label:"Career Simulate" },
  { path:"/roadmap",  query:"",          icon:"→", label:"Roadmap"         },
  { path:"/interview",query:"",          icon:"◷", label:"Interview Prep"  },
  { path:"/pricing",  query:"",          icon:"↑", label:"Upgrade"         },
];

const BUSINESS_NAV = [
  { path:"/business",    query:"", icon:"⊞", label:"Dashboard"        },
  { path:"/jd",          query:"", icon:"▤", label:"JD Generator"     },
  { path:"/hiring",      query:"", icon:"◎", label:"Hiring AI"        },
  { path:"/team-skills", query:"", icon:"◈", label:"Team Skill Map"   },
  { path:"/workforce",   query:"", icon:"⬡", label:"Workforce Plan"   },
  { path:"/salary",      query:"", icon:"₹", label:"Salary Benchmark" },
  { path:"/interview-ai",query:"", icon:"◷", label:"Interview AI"     },
  { path:"/pricing",     query:"", icon:"↑", label:"Upgrade"          },
];
            ...
            {OTHER_APPS.map(app => {
              ...
              return (
                <a key={app.url}
                  ...
                  style={{ width:30, height:30, borderRadius:8, background:`${app.color}12`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, color:app.color, textDecoration:"none", opacity: allowed ? 1 : 0.5, position:"relative" }}>
                  {app.icon}
                  {!allowed && <span style={{ position:"absolute", bottom:-2, right:-2, fontSize:8 }}>🔒</span>}
                </a>
              );
            })}
          </div>
