...
const CARD  = "#ffffff";
const BORDER= "#e8eaed";
const API   = "https://nugens-platform-production.up.railway.app";
...
export default function BookServices({ profile }) {
  ...
  const pay = async () => {
    ...
    try {
      ...
      // FIX: previously sent plan:`units_${service}_${pkg}` to the generic
      // subscription endpoint, which only matches fixed plan IDs like
      // "units_pro_monthly" — every booking payment failed with "Unknown
      // plan". These dedicated endpoints validate against the server-side
      // catalog (UNITS_SERVICE_CATALOG) instead.
      const res = await fetch(`${API}/api/units/bookings/create-order`, {
        method:"POST",
        headers: authHeaders,
        body: JSON.stringify({ serviceId:service, packageName:pkg })
      });
      const order = await res.json();
      if (!res.ok) throw new Error(order?.error || "Couldn't start payment. Please try again.");
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response) => {
          // FIX: booking details (service, package, contact, project notes)
          // were previously never sent to the backend at all — payment
          // succeeded but Nugens had no record of what was booked.
          try {
            const vRes = await fetch(`${API}/api/units/bookings/verify`, {
              method:"POST",
              headers: authHeaders,
              body: JSON.stringify({
                ...response,
                serviceId: service,
                packageName: pkg,
                name: form.name, email: form.email, phone: form.phone, company: form.company, note: form.note,
              })
            });
            if (!vRes.ok) throw new Error("Payment succeeded but booking could not be saved — contact support with your payment ID.");
            const vData = await vRes.json();
            setSuccess(true);
            setLoading(false);
            if (vData?.booking?.id) {
              setTimeout(() => navigate(`/receipt/${vData.booking.id}`), 900); // brief success state, then the full receipt
            }
            return;
          } catch (e) {
      ...
      console.error(e);
      setBookErr(e.message || "Something went wrong. Please try again.");
      setLoading(false);
    }
  };
    ...
    <div style={S.page}>
      ...
      {step===3 && (
        <div style={{ maxWidth:520 }}>
          ...
          {bookErr && (
            <div style={{ background:"#dc262610", border:"1px solid #dc262630", borderRadius:9, padding:"10px 14px", marginBottom:14, color:"#dc2626", fontSize:13 }}>
              ⚠ {bookErr}
            </div>
          )}
          <div style={{ display:"flex", gap:12 }}>
