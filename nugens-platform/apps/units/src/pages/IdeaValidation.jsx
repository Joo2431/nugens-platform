...
import { miniChat, apiPost } from "../lib/apiClient";
...
export default function IdeaValidation({ profile }) {
  ...
  // BUG FIX: this previously just called setBooked(true) — a local state
  // flip with no backend call, no record, and nobody actually notified.
  // Now it creates a real request the team can see and follow up on.
  const bookConsultation = async () => {
    setBooking(true);
    setBookError("");
    try {
      await apiPost("/api/units/consultation-requests", {
        ideaSummary: idea.slice(0, 2000),
        score: result?.score,
        contactNote: `Verdict: ${result?.verdict || "n/a"}`,
      });
      setBooked(true);
    } catch (e) {
      setBookError(e.message || "Couldn't submit your request. Please try again.");
    } finally {
      setBooking(false);
    }
  };
...
JSON format:
{
  ...
  "summary": "<3 clear sentences of overall assessment>"
      ...
      try {
        ...
        if (typeof parsed.score !== "number") throw new Error("Malformed response");
      } catch {
        // BUG FIX: this used to silently fabricate a plausible-looking result
        // (score: 65, generic strengths/risks) whenever the AI response
        // failed to parse — meaning the user could get a completely made-up
        // score with no indication it wasn't real. Now it fails honestly so
        // the person knows to just retry rather than trusting a fake number.
        throw new Error("The assessment couldn't be generated properly. Please try again.");
      }
  ...
  return (
    <div style={S.page}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); .un-iv-layout{display:grid;grid-template-columns:1fr 300px;gap:28px} @media(max-width:800px){.un-iv-layout{grid-template-columns:1fr!important}}`}</style>

      <div className="un-iv-layout">
        <div>
            ...
            <div>
              ...
              <div style={{ ...S.card, marginBottom:16, display:"grid", gridTemplateColumns:"auto 1fr", gap:24, alignItems:"center" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:11, color:MUTED, marginTop:10, lineHeight:1.5 }}>
                    ⓘ This is an AI-generated assessment based on general market patterns, not live competitor or funding data. Use it as a starting signal — the premium consultation below adds a human review.
                  </div>
                </div>
              </div>
              ...
              {!consultBooked ? (
                <div style={{ background:"#fef2f2", border:`1px solid ${PINK}20`, borderRadius:14, padding:24 }}>
                  ...
                  {bookError && <div style={{ fontSize:12, color:"#dc2626", marginBottom:12 }}>{bookError}</div>}
                  <div style={{ display:"flex", gap:10 }}>
                    <button onClick={bookConsultation} disabled={booking} style={{ ...S.btn, opacity: booking ? 0.7 : 1 }}>
                      {booking ? "Submitting…" : "Book Premium Consultation — ₹999"}
                    </button>
                ...
                <div style={{ background:"#f0fdf4", border:"1px solid #bbf7d0", borderRadius:14, padding:20, textAlign:"center" }}>
                  <div style={{ fontSize:16, fontWeight:700, color:GREEN, marginBottom:6 }}>✓ Request Submitted!</div>
