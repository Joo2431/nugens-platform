...
import { miniChat, apiGet, apiPost } from "../lib/apiClient";
...
export default function EntrepreneurGuide({ profile }) {
  ...
  useEffect(() => {
    apiGet("/api/units/guide-progress").then(d => setCompleted(d.completed || [])).catch(() => {});
  }, []);
  ...
  const markDone = (chId, secIdx) => {
    ...
    if (!completed.includes(key)) {
      ...
      setCompleted(next);
      apiPost("/api/units/guide-progress", { completed: next }).catch(() => {}); // best-effort save
    }
  };
  ...
  const askAI = async () => {
    ...
    try {
      const d = await miniChat(
        `You are a startup mentor at Nugens. Answer this question from an aspiring entrepreneur in India in a practical, encouraging, and specific way. Topic: ${chapter?.title}. Question: ${aiQuestion}`,
        "units", "individual"
      );
