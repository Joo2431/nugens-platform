...
export default function ImageGenerator({ profile }) {
  ...
  // Load image library from Supabase on mount
  useEffect(() => {
    if (!profile?.id) { setHistLoad(false); return; }
    supabase
      .from("dh_generated_images")
      .select("*")
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (profile?.id) {
        ...
        supabase.from("dh_analytics_events").insert({ user_id:profile.id, event_type:"image_generated", platform:purpose }).then(()=>{});
      }
    } catch(err) {
  ...
  const deleteImage = async (id) => {
    ...
    if (history.find(img => img.id === id)?.image_url === imgUrl) setImgUrl(null);
    setDeleting(null);
  };
  ...
  return (
    <div style={{ fontFamily:"'Plus Jakarta Sans',sans-serif", padding:"28px 32px", background:LIGHT, minHeight:"100vh" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); .img-thumb{transition:all 0.15s} .img-thumb:hover{transform:scale(1.04);box-shadow:0 4px 16px rgba(0,0,0,0.12)} @keyframes bounce{0%,80%,100%{transform:translateY(0)}40%{transform:translateY(-8px)}}`}</style>
        ...
        <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:16, padding:24, display:"flex", flexDirection:"column", gap:18 }}>
          <div>
            ...
            <textarea value={prompt} onChange={e=>setPrompt(e.target.value)} rows={5}
              ...
              style={{ width:"100%", padding:"11px 14px", border:`1px solid ${BORDER}`, borderRadius:10, fontSize:13, color:TEXT, fontFamily:"inherit", resize:"vertical", outline:"none", boxSizing:"border-box" }}
              onFocus={e=>e.target.style.borderColor=PINK+"60"} onBlur={e=>e.target.style.borderColor=BORDER}
          ...
          <p style={{ fontSize:11, color:MUTED, margin:0, textAlign:"center" }}>Ctrl+Enter to generate</p>
        </div>
      ...
      <div style={{ marginTop:36, maxWidth:1100 }}>
        ...
        {!histLoad&&history.length===0&&(
          <div style={{ padding:24, background:CARD, border:`1px solid ${BORDER}`, borderRadius:12, textAlign:"center", color:MUTED, fontSize:13 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(110px,1fr))", gap:12 }}>
            {history.map(img=>(
              <div key={img.id} className="img-thumb"
                style={{ borderRadius:10, overflow:"hidden", border:`1px solid ${BORDER}`, cursor:"pointer", position:"relative", background:CARD }}>
                <img src={img.image_url} alt={img.prompt_short||""}
                  onClick={()=>{ setImgUrl(img.image_url); setPrompt(img.prompt||""); setError(null); }}
                  style={{ width:"100%", height:80, objectFit:"cover", display:"block" }}
                  onError={e=>{ e.target.style.background="#f3f4f6"; e.target.style.height="80px"; }}/>
                ...
                <button onClick={()=>deleteImage(img.id)} disabled={deleting===img.id} title="Delete"
                  style={{ position:"absolute", top:4, right:4, width:18, height:18, background:"rgba(0,0,0,0.55)", border:"none", borderRadius:"50%", color:"#fff", fontSize:10, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  ✕
                </button>
              </div>
            ))}
