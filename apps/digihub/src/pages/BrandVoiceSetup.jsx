...
export default function BrandVoiceSetup({ profile }) {
  ...
  // BUG FIX: Brand Voice previously lived almost entirely in localStorage
  // (only `industry` reached Supabase), so no backend tool could ever read
  // it — Image Generator, Bulk Generator, etc. had no way to apply it.
  // Now the backend (dh_brand_voice table) is the source of truth; this
  // just hydrates from it on load, with localStorage as an instant-paint cache.
  useEffect(() => {
    apiGet("/api/digihub/brand-voice").then(({ brandVoice }) => {
      if (brandVoice) {
        const hydrated = {
          brandName: brandVoice.brand_name || "",
          industry: brandVoice.industry || profile?.industry || "",
          tone: brandVoice.tone || "",
          audience: brandVoice.audience || "",
          usp: brandVoice.usp || "",
          platforms: brandVoice.platforms || [],
          postFreq: brandVoice.post_freq || "",
          brandKeywords: brandVoice.brand_keywords || "",
          avoidWords: brandVoice.avoid_words || "",
          emoji: brandVoice.emoji_style || "moderate",
        };
        setVoice(hydrated);
  ...
  const save = async () => {
    ...
    try {
      await apiPost("/api/digihub/brand-voice", {
        ...
        brandKeywords: voice.brandKeywords,
        emoji: voice.emoji,
        postFreq: voice.postFreq,
      });
    } catch(e) { console.error("Brand voice save failed:", e.message); }
    setSaving(false);
    setSaved(true);
  ...
  return (
    <div style={{minHeight:"100vh",background:BG,padding:"32px 36px",fontFamily:"'Plus Jakarta Sans',sans-serif",color:TEXT}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); input:focus,select:focus,textarea:focus{border-color:${BLUE}!important;outline:none} .dh-bv-grid{display:grid;grid-template-columns:1fr 1fr;gap:20px;max-width:1100px} @media(max-width:800px){.dh-bv-grid{grid-template-columns:1fr!important}}`}</style>
