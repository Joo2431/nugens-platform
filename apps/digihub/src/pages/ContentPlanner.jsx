...
export default function ContentPlanner({ profile }) {
  ...
  const generatePlan = async () => {
    if (!topic.trim()) { setTopicTouched(true); return; }
    // Real gating — the pricing page says free tier is "Content Planner
    // (manual only)"; previously nothing enforced that and free users
    // could call the AI planner same as paid tiers.
    if ((profile?.plan || "free") === "free") {
      setError("AI content planning is a paid feature — the free plan is manual-only. Upgrade to generate plans automatically.");
      return;
    }
  ...
  return (
    <div style={{minHeight:"100vh",background:BG,padding:"32px 36px",fontFamily:"'Plus Jakarta Sans',sans-serif",color:TEXT}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); input:focus,select:focus,textarea:focus{border-color:${PINK}!important;outline:none} .dh-cp-layout{display:grid;grid-template-columns:1fr 340px;gap:24px} @media(max-width:820px){.dh-cp-layout{grid-template-columns:1fr!important}}`}</style>
              ...
              {Array(daysInMonth).fill(null).map((_,i)=>{
                ...
                return (
                  <div key={day} onClick={()=>setSelected(day)}
                    style={{
                      background: selected===day ? `${PINK}10` : (dayPosts.length>0?`${BLUE}08`:CARD),
                      border: selected===day ? `2px solid ${PINK}` : (isToday?`2px solid ${BLUE}`:`1px solid ${BORDER}`),
                      borderRadius:8,minHeight:68,padding:4,cursor:"pointer",position:"relative",
                      transition:"all 0.13s",
                      boxShadow: selected===day ? `0 0 0 3px ${PINK}18` : "none",
                    }}>
                    <div style={{fontSize:10,fontWeight:700,color:selected===day?PINK:(isToday?BLUE:MUTED),marginBottom:3,padding:"1px 3px"}}>{day}</div>
          ...
          {addModal && (
            <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}
              ...
              <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:16,padding:24,width:"100%",maxWidth:440,boxShadow:"0 16px 48px rgba(0,0,0,0.15)"}}>
                ...
                <textarea value={newPostText} onChange={e=>setNewPostText(e.target.value.slice(0,2200))} rows={4}
                  maxLength={2200}
                ...
                <div style={{textAlign:"right",fontSize:11.5,fontWeight:600,marginTop:5,marginBottom:16,
                  color: newPostText.length>=2200 ? "#dc2626" : (newPostText.length>1900 ? "#d97706" : MUTED)}}>
                  {newPostText.length}/2200
                </div>
        ...
        <div>
          <div style={{background:CARD,border:`1px solid ${BORDER}`,borderRadius:14,padding:24,boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
            ...
            <input value={topic} onChange={e=>setTopic(e.target.value)} placeholder="e.g. Diwali sale, New product launch…"
              style={{width:"100%",background:BG,border:`1.5px solid ${!topic.trim()&&topicTouched?"#fca5a5":BORDER}`,borderRadius:8,padding:"9px 12px",color:TEXT,fontSize:13,fontFamily:"inherit",outline:"none",boxSizing:"border-box"}}
              onBlur={()=>setTopicTouched(true)}/>
            {!topic.trim() && topicTouched && (
              <div style={{fontSize:11,color:"#dc2626",marginTop:5,marginBottom:10}}>Please enter a theme or topic before generating.</div>
            )}
            <div style={{marginBottom: !topic.trim()&&topicTouched ? 6 : 16}}/>

            <button onClick={generatePlan} disabled={loading}
              style={{width:"100%",boxSizing:"border-box",padding:"13px 0",background:loading?`${PINK}60`:PINK,color:"#fff",border:"none",borderRadius:9,fontSize:13.5,fontWeight:700,cursor:loading?"not-allowed":"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:7,boxShadow:loading?"none":`0 2px 10px ${PINK}40`}}>
              <span style={{fontSize:14}}>⚡</span>
              {loading?"Generating…":"Generate & Auto-Place"}
            </button>

            <button onClick={clearCalendar}
              style={{width:"100%",boxSizing:"border-box",marginTop:8,padding:"9px 0",background:"none",border:"none",borderRadius:9,fontSize:12,fontWeight:600,color:"#9ca3af",cursor:"pointer",fontFamily:"inherit",textAlign:"center"}}
            ...
            {error&&<div style={{fontSize:11.5,color:"#dc2626",textAlign:"center",marginTop:8,lineHeight:1.5}}>{error}</div>}
          </div>
