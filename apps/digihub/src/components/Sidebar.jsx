...
// DigiHub now uses white/pink to match HyperX and Units
const PINK   = "#e8185d";
const BORDER = "#e8eaed";
const MUTED  = "#9ca3af";
const TEXT   = "#111827";
  ...
  return (
    <>
      <style>{`
        ...
        .dh-nav.active .dh-ico { color:${PINK}; }
        .dh-nav:hover .dh-ico { color:${PINK}; }
        .dh-ico { font-size:14px; flex-shrink:0; width:18px; text-align:center; color:#d1d5db; }
        .dh-signout { width:100%; padding:9px 13px; background:none; border:1px solid #e8eaed;
      ...
      <div style={{
        ...
        {!collapsed && (
          <div style={{ background: isBusiness ? "#fef2f2" : "#f0fdf4", border:`1px solid ${isBusiness ? PINK+"30" : "#bbf7d0"}`, borderRadius:9, padding:"7px 11px", marginBottom:16 }}>
            ...
            <div style={{ fontSize:10, color:MUTED }}>{isBusiness ? "Brand tools & content" : "Portfolio & job discovery"}</div>
          </div>
        )}
