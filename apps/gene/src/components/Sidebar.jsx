...
/**
 * MERGED SIDEBAR — shows ALL nav items in one list.
 * Individual tools are grouped under a subtle header,
 * Business tools appear when in business mode.
 * This replaces the two-section layout that was wasting space.
 */
...
function NavItem({ item, collapsed }) {
  ...
  const isActive = item.path === "/chat"
    ? item.query
      ? location.pathname === "/chat" && location.search.includes(`t=${item.query}`)
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius:9, fontSize:13, fontWeight: isActive ? 700 : 500,
    ...
    >
      <span style={{ fontSize:13, flexShrink:0, color: isActive ? PINK : "#ccc", width:16, textAlign:"center" }}>
        {item.icon}
      </span>
  ...
  return (
    <>
      <style>{`
        ...
        .gn-mode-sw { width:100%; border:none; border-radius:9px; padding:7px 11px;
          display:flex; align-items:center; justify-content:space-between; gap:6px;
          font-family:'Plus Jakarta Sans',sans-serif; transition:all 0.15s; cursor:pointer; }
        .gn-sw-item { display:flex; align-items:center; gap:9px; padding:9px 12px;
        ...
        .gn-signout { width:100%; padding:8px 12px; background:none; border:1px solid #f0f0f0;
      ...
      <div style={{
        ...
        {/* Logo row */}
        <div style={{ display:"flex", alignItems:"center", justifyContent: collapsed ? "center" : "space-between", marginBottom:16, paddingLeft: collapsed ? 0 : 4 }}>
          {!collapsed && (
          ...
          <div style={{ position:"relative", marginBottom:10 }}>
            ...
            >
              <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                ...
                <div>
                  <div style={{ fontSize:9, fontWeight:800, textTransform:"uppercase", letterSpacing:"0.1em", color: isBusiness ? "#0284c7" : PINK }}>
        ...
        {/* User chip — compact */}
        {!collapsed && (
          <div style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 10px", background:"#fafafa", borderRadius:8, marginBottom:10 }}>
            <div style={{ width:24, height:24, borderRadius:"50%", background:`${PINK}15`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:9, fontWeight:800, color:PINK, flexShrink:0 }}>
            ...
            <div style={{ overflow:"hidden" }}>
              ...
              <div style={{ fontSize:9, color:"#bbb", textTransform:"capitalize" }}>{plan}</div>
            </div>
          </div>
        )}

        {/* ALL Nav items in one list — no dual section */}
        <nav style={{ flex:1, display:"flex", flexDirection:"column", gap:1 }}>
          {nav.map((n, i) => (
            <NavItem key={i} item={n} collapsed={collapsed} />
          ))}
        </nav>

        {/* Free plan upgrade nudge */}
        {!collapsed && plan === "free" && (
          <div style={{ background:"#fff5f7", border:"1px solid #fce", borderRadius:10, padding:"9px 11px", marginBottom:10 }}>
