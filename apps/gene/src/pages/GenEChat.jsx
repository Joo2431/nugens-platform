...
// ─── FIX 1: ALL paid plans — ng_*, gene_*, hx_*, dh_*, units_* ─────────────
const PAID_PLANS = new Set([
  "monthly","yearly","gene_monthly","gene_yearly","admin",
  ...
  "dh_starter","dh_premium","dh_pro","dh_monthly","dh_yearly",
  "units_starter","units_pro","gene_biz_starter","gene_biz_pro",
]);

// ─── FIX 2: plans that can access live job search (matches server PAID_JOB_PLANS) ─
const JOB_PLANS = new Set([
  "yearly","gene_yearly","admin",
  "ng_ind_starter","ng_ind_premium","ng_ind_pro",
  "ng_biz_starter","ng_biz_premium","ng_biz_pro",
]);
...
const INDIVIDUAL_FEATURES = [
  ...
  { key:"resume",    icon:"▤", color:"#7c3aed", bg:"#f5f3ff", label:"ATS Resume",      desc:"Build & optimise your resume for ATS",           path:"/resume",    pro:true },
  { key:"skill_gap", icon:"◈", color:"#0284c7", bg:"#eff6ff", label:"Skill Gap",       desc:"Find the gap between you and your target role",  path:"/skill-gap" },
  { key:"simulate",  icon:"⬡", color:"#d97706", bg:"#fff7ed", label:"Career Simulate", desc:"Model salary, timeline & risks of any career move",path:"/simulate" },
  { key:"roadmap",   icon:"→", color:"#16a34a", bg:"#f0fdf4", label:"Roadmap",         desc:"Phase-by-phase career plan for your goal",       path:"/roadmap" },
  { key:"interview", icon:"◷", color:PINK,      bg:"#fff0f4", label:"Interview Prep",  desc:"Mock interviews & preparation strategy",         path:"/interview", pro:true },
...
const BUSINESS_FEATURES = [
  { key:"jd",        icon:"▤", color:"#7c3aed", bg:"#f5f3ff", label:"JD Generator",   desc:"Full job descriptions + interview questions",    path:"/jd" },
  { key:"hiring",    icon:"◎", color:PINK,      bg:"#fff0f4", label:"Hiring AI",       desc:"Skills, salary & full hiring strategy",          path:"/hiring" },
  { key:"team",      icon:"◈", color:"#0284c7", bg:"#eff6ff", label:"Team Skill Map",  desc:"Analyse team skills, find gaps, plan training",  path:"/team-skills" },
  { key:"workforce", icon:"⬡", color:"#d97706", bg:"#fff7ed", label:"Workforce Plan",  desc:"Hiring roadmap based on your company stage",     path:"/workforce" },
  { key:"salary",    icon:"₹", color:"#16a34a", bg:"#f0fdf4", label:"Salary Benchmark",desc:"Real Indian market salary data by experience",   path:"/salary" },
  { key:"interview", icon:"◷", color:"#0284c7", bg:"#eff6ff", label:"Interview AI",    desc:"Role-specific questions + evaluation rubric",    path:"/interview-ai" },
];
...
function AppSwitcherBar({ profile }) {
  ...
  return (
    <div style={{
      ...
      boxShadow:"0 1px 0 #f3f4f6", gap:8,
    }}>
      <style>{`.gene-appswitcher-scroll::-webkit-scrollbar{display:none}`}</style>
      ...
      {/* BUG FIX: this row (logo, app pills, Dashboard/Upgrade) had no overflow
          handling at all — on narrow phones the pill list ran past the screen
          edge and the last app ("Units") got physically clipped off-screen.
          The app pills now scroll horizontally on their own; the logo and the
          Dashboard/Upgrade buttons stay fixed and always fully visible. */}
      <div className="gene-appswitcher-scroll" style={{ display:"flex", alignItems:"center", gap:4, overflowX:"auto", minWidth:0, scrollbarWidth:"none", msOverflowStyle:"none" }}>
        {APPS.map(app => (
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
        {/* BUG FIX: this was <a href="https://nugens.in.net/dashboard">, an
            absolute link to a completely different subdomain/app instead of
            this app's own Dashboard route ("/"). Clicking it took people off
            Gen-E entirely. Now a proper in-app navigation button. */}
        <button onClick={() => navBar("/")}
          style={{ fontSize:11, fontWeight:600, color:"#9ca3af", background:"none", textDecoration:"none", padding:"4px 10px", border:"1px solid #e8eaed", borderRadius:7, transition:"all 0.15s", cursor:"pointer", fontFamily:"inherit" }}
...
function PlanBadge({ plan }) {
  ...
  if (plan === "yearly" || plan === "gene_yearly") return <span style={{fontSize:10,fontWeight:700,color:"#7c3aed",letterSpacing:"0.04em"}}>Pro ✦</span>;
...
function UpgradeModal({ feature, onClose, onUpgrade }) {
  const COPY = {
    ...
    job_search:{icon:"🔎",title:"Yearly Plan — Job Match",body:"Live job matching requires a Pro Yearly or Nugens Suite plan."},
  };
  ...
  const downloadResumePDF = (txt) => {
    try {
      ...
      const html = txt
        ...
        .map(line => {
          const l = line.trim();
          if (!l) return "<div style=\"margin:6px 0\"></div>";
          if (l.startsWith("## ")) {
            const h = l.replace(/^## /, "");
            return `<div style="margin-top:22px;padding-bottom:5px;border-bottom:2px solid #e8185d;">
              <span style="font-size:9pt;font-weight:800;color:#e8185d;text-transform:uppercase;letter-spacing:0.09em">${h}</span>
            </div>`;
          }
          if (l.startsWith("### ")) {
...
<head>
  <meta charset="UTF-8"/>
  <title>Resume — Gen-E AI</title>
  <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap');
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Inter', Arial, sans-serif; background: #fff; color: #111; }
    .page { width: 794px; min-height: 1123px; margin: 0 auto; padding: 0 0 40px; }
    .header { background: #e8185d; color: #fff; padding: 18px 44px; display: flex; justify-content: space-between; align-items: center; }
    .header-brand { font-size: 20pt; font-weight: 900; letter-spacing: -0.03em; }
    .header-sub { font-size: 8pt; opacity: 0.85; margin-top: 2px; }
    .header-date { font-size: 8pt; opacity: 0.85; text-align: right; }
    .content { padding: 28px 44px; }
    .footer { text-align: center; font-size: 7pt; color: #bbb; padding: 16px 44px 0; border-top: 1px solid #f0f0f0; margin-top: 32px; }
    @media print {
...
<body>
  <div class="page">
    <div class="header">
      <div>
        <div class="header-brand">GEN-E</div>
        <div class="header-sub">AI Career Intelligence · nugens.in.net</div>
      </div>
      <div class="header-date">AI-Generated Resume<br/>${date}</div>
    </div>
    <div class="content">${html}</div>
    <div class="footer">Generated by Gen-E AI · nugens.in.net · This resume was created with AI assistance</div>
  </div>
  <script>
  ...
  const triggerFeature = (f) => {
    ...
    // Route to the tool's real page when it has one — only features without
    // a dedicated standalone page (Career AI itself, Readiness Score, Job
    // Match) still run as an in-chat conversation.
    if (f.path) { nav(f.path); return; }
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%",background:"#fff",borderRight:"1px solid #f0f0f0",overflow:"hidden"}}>
              ...
              {list.map(chat=>{
                ...
                return (
                  <div key={chat.id} onClick={()=>{setActiveId(chat.id);setSidebarOpen(false);}}
                    ...
                    <button className="sb-chat-menu-btn" onClick={e=>{e.preventDefault();e.stopPropagation();setCtx({x:e.clientX,y:e.clientY,id:chat.id});}}
                      title="Chat options"
                      style={{background:"none",border:"none",cursor:"pointer",fontSize:13,color:"#bbb",padding:"2px 5px",flexShrink:0,borderRadius:5,lineHeight:1}}
                      onMouseEnter={e=>{e.currentTarget.style.background="#eee";e.currentTarget.style.color="#888";}}
      ...
      <div style={{padding:"10px 12px 6px",borderTop:"1px solid #f5f7fa",flexShrink:0}}>
        <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:10}}>
          {[
            {icon:"⊞",label:"Dashboard",path:"/",pro:false},
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:5,marginBottom:10}}>
          {[
            {label:"ATS Resume",    path:"/resume",     pro:true},
            {label:"Career Plan",   path:"/roadmap",     pro:false},
            {label:"Skill Gap",     path:"/skill-gap",   pro:false},
            {label:"Mock Interview",path:"/interview",   pro:true},
            ...
            const plan = profile?.plan||"free";
            // FIX 2: use JOB_PLANS for yearly gate — allows ng_* suite plan users
            const locked = (t.pro && !PAID_PLANS.has(plan)) || (t.yearly && !JOB_PLANS.has(plan));
            const handleTool = () => {
              if(locked){ setUpgrade(t.yearly?"job_search":"resume_builder"); return; }
              // Tools with a dedicated page navigate there; only the two
              // tools without a standalone page (Score Me, Job Match) still
              // run as an in-chat conversation.
              if (t.path) { nav(t.path); setSidebarOpen(false); return; }
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .sb-chat-menu-btn{opacity:0;transition:opacity 0.12s}
        .sb-chat-row:hover .sb-chat-menu-btn,.sb-chat-menu-btn:focus{opacity:1}
        @media (hover:none){.sb-chat-menu-btn{opacity:1}}
                  ...
                  {active?.messages.map((msg,i)=>(
                    <div key={i} className="msg-in" style={{display:"flex",justifyContent:msg.role==="user"?"flex-end":"flex-start",alignItems:"flex-start",gap:10}}>
                      ...
                      <div style={{maxWidth:"80%",display:"flex",flexDirection:"column",alignItems:msg.role==="user"?"flex-end":"flex-start",gap:5}}>
                          ...
                          {msg.role==="user" && editIdx!==i && (
                            <button onClick={()=>startEdit(i, msg.text)}
                              ...
                              style={{position:"absolute",top:6,left:-32,width:24,height:24,
                                opacity:0,background:"rgba(0,0,0,0.07)",border:"none",
                        ...
                        {/* FIX 3: JobCards rendered OUTSIDE bubble using the component */}
                        {msg.role==="assistant" && !msg.streaming && msg.jobs && msg.jobs.length > 0 && (
                          <JobCards jobs={msg.jobs}/>
                        )}
            ...
            <div style={{padding:"10px 20px 18px",background:"#fff",flexShrink:0,borderTop:"1px solid #f0f2f5"}}>
              <div style={{maxWidth:720,margin:"0 auto"}}>
                ...
                <div style={{display:"flex",alignItems:"flex-end",gap:8,background:"#f8fafb",border:"1.5px solid #edf0f3",borderRadius:16,padding:"8px 8px 8px 14px",transition:"all 0.15s"}}
                  ...
                  <textarea ref={inputRef} value={input} rows={1}
                    ...
                    onPaste={e=>{
                      ...
                      if(input.length+pasted.length>MAX_CHARS){
                        e.preventDefault();
                        setInput(v=>(v+pasted).slice(0,MAX_CHARS));
                      }
                    }}
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:7,flexWrap:"wrap",gap:6}}>
                  ...
                  <div style={{fontSize:10.5,color:input.length>MAX_CHARS*0.9?(input.length>=MAX_CHARS?"#e8185d":"#d97706"):"#d0d5dd",display:"flex",alignItems:"center",gap:10}}>
