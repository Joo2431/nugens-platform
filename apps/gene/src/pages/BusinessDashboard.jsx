...
export default function BusinessDashboard({ profile }) {
  ...
  return (
    <div style={{padding:"32px 36px",background:"#f8f9fb",minHeight:"100vh",fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); .bt:hover{transform:translateY(-2px);box-shadow:0 8px 32px rgba(0,0,0,0.1)!important} @keyframes su{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}} .su{animation:su 0.3s ease both}`}</style>

      {/* Header */}
      <div style={{marginBottom:32}} className="su">
        <div style={{display:"inline-flex",alignItems:"center",gap:6,padding:"3px 10px",borderRadius:5,background:"#fef2f2",border:"1px solid #fecaca",marginBottom:14}}>
          <span style={{fontSize:10,fontWeight:700,color:"#e8185d",textTransform:"uppercase",letterSpacing:"0.08em"}}>🏢 Business Mode</span>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:32}} className="su">
        {[
          ...
          { label:"Integrations",val:"2",     sub:"Gen-E · HyperX · Digi", color:"#d97706" },
        ].map(s=>(
          <div key={s.label} style={{background:"#ffffff",border:"1px solid #e8eaed",borderRadius:12,padding:"16px 18px",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
            <div style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.07em",color:"#6b7280",marginBottom:6}}>{s.label}</div>
      ...
      {/* Upgrade banner */}
      {plan === "free" && (
        <div style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:14,padding:"18px 22px",marginBottom:28,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}} className="su">
          <div>
            <div style={{fontSize:14,fontWeight:700,color:"#111827",marginBottom:3}}>Unlock Business Intelligence features</div>
            <div style={{fontSize:12.5,color:"#6b7280"}}>Team skill mapping, workforce planning and candidate matching require Pro.</div>
          </div>
      ...
      {/* Tools grid */}
      <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"#6b7280",marginBottom:14}}>Workforce AI Tools</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:14}}>
        {TOOLS.map((t,i)=>(
          <Link key={t.to} to={t.to} style={{background:"#ffffff",border:"1px solid #e8eaed",borderRadius:14,padding:"20px",boxShadow:"0 1px 3px rgba(0,0,0,0.04)",textDecoration:"none",display:"block",transition:"all 0.18s",animationDelay:`${i*50}ms`}} className="bt su">
            ...
            <div style={{fontSize:14,fontWeight:700,color:"#111827",marginBottom:4}}>{t.label}</div>
