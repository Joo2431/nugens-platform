...
const BUSINESS_NAV = [
  ...
  { to:"/feed",     icon:"◉",  label:"Community"        },
  { to:"/guidance", icon:"✦",  label:"AI Guidance"      },
  { to:"/book",     icon:"◈",  label:"Book Our Services"},
  { to:"/quote",    icon:"◈",  label:"Book an Event"    },
  { to:"/tracker",  icon:"☑",  label:"Project Tracker"  },
  { to:"/portfolio",icon:"◇",  label:"Our Work"         },
...
export default function Sidebar({ profile, user, onSignOut, open, onClose }) {
  ...
  const navWithAdmin = plan === "admin" ? [...nav, { to:"/admin/projects", icon:"⊞", label:"Projects (Admin)" }] : nav;
            ...
            {OTHER_APPS.map(app => {
              ...
              return (
                <a key={app.url}
                  ...
                  className="u-app-link" style={{ opacity: allowed ? 1 : 0.55 }}>
