...
const PRODUCTS = [
  ...
  { icon:"◇", label:"Units",    sub:"Visual production house",          color:"#d97706" },
];
...
export default function AuthPage() {
  ...
  const [forgotView,    setForgotView]    = useState(false);  // forgot password screen
      ...
      <div style={{ minHeight:"100vh", display:"flex" }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"32px 24px", background:"#fff", minWidth:0 }}>
          <div style={{ width:"100%", maxWidth:400 }} className="fu">
            ...
            {forgotView && (
              <div style={{ background:"#fff", border:"1.5px solid #f0f0f0", borderRadius:16, padding:"28px", boxShadow:"0 4px 32px rgba(0,0,0,0.05)", marginBottom:0 }}>
                {forgotSent ? (
                  <div style={{ textAlign:"center" }}>
                    <div style={{ fontSize:40, marginBottom:16 }}>📧</div>
                    <div style={{ fontWeight:700, fontSize:16, color:"#0a0a0a", marginBottom:8 }}>Reset link sent!</div>
                    ...
                    <button onClick={() => setForgotSent(false)}
                      style={{ background:"none", border:"none", color:PINK, fontWeight:600, fontSize:12.5, cursor:"pointer", fontFamily:"inherit", textDecoration:"underline", marginRight:16 }}>
                      Resend link
                    </button>
                    <button onClick={() => { setForgotView(false); setForgotSent(false); }}
                      style={{ background:"none", border:"none", color:"#9ca3af", fontWeight:500, fontSize:12.5, cursor:"pointer", fontFamily:"inherit" }}>
                      ← Back to sign in
                    </button>
                  </div>
                ) : (
                  <>
                    <div style={{ fontWeight:700, fontSize:15, color:"#0a0a0a", marginBottom:6 }}>Reset your password</div>
                    <p style={{ fontSize:13, color:"#9ca3af", marginBottom:20, lineHeight:1.6 }}>Enter your email and we'll send you a reset link.</p>
                    <form onSubmit={handleForgot} style={{ display:"flex", flexDirection:"column", gap:14 }}>
                      <div>
                        <label style={{ fontSize:11.5, fontWeight:600, color:"#374151", display:"block", marginBottom:5 }}>Email Address</label>
                        <input className="ai" type="email" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)} placeholder="you@email.com" required autoFocus />
                      </div>
                      {error && <div style={{ background:"#fff5f8", border:"1px solid #ffd0de", borderRadius:8, padding:"10px 13px", fontSize:12.5, color:PINK }}>⚠️ {error}</div>}
                      <button type="submit" disabled={forgotLoading}
                        style={{ width:"100%", padding:"11px 0", background:forgotLoading?"#f3f4f6":"#0a0a0a", border:"none", borderRadius:10, fontFamily:"inherit", fontWeight:700, fontSize:13.5, color:forgotLoading?"#9ca3af":"#fff", cursor:forgotLoading?"not-allowed":"pointer" }}>
                        {forgotLoading ? "Sending…" : "Send Reset Link →"}
                      </button>
                      <button type="button" onClick={() => { setForgotView(false); setError(""); }}
                        style={{ background:"none", border:"none", color:"#9ca3af", fontSize:12.5, cursor:"pointer", fontFamily:"inherit", textAlign:"center" }}>
                        ← Back to sign in
                      </button>
                    </form>
                  </>
              ...
              <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
                ...
                <div>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:5 }}>
                    <label style={{ fontSize:11.5, fontWeight:600, color:"#374151" }}>Password</label>
                    {tab==="login" && (
                      <button type="button" onClick={() => { setForgotView(true); setError(""); setForgotSent(false); setForgotEmail(form.email || ""); }}
                        style={{ background:"none", border:"none", color:"#e8185d", fontWeight:600, fontSize:11.5, cursor:"pointer", fontFamily:"inherit", padding:0 }}>
                        Forgot password?
                      </button>
                    )}
                  </div>
