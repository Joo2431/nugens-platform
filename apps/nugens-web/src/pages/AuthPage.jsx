...
export default function AuthPage() {
  ...
  // Where to go after login — from ?redirect= param or default to /
  ...
  const goAfterLogin = () => {
    // If redirect is an external URL (another subdomain), do full navigation
  ...
  useEffect(() => {
    // Handle hash token from Google OAuth
  ...
  const handleGoogle = async () => {
    ...
    const { error: e2 } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `https://nugens.in.net/auth?redirect=${encodeURIComponent(redirect)}` }
    });
        ...
        <div className="al" style={{ width:"42%", flexShrink:0, background:"#0a0a0a", flexDirection:"column", position:"relative", overflow:"hidden" }}>
          ...
          <div style={{ position:"relative", zIndex:2, display:"flex", flexDirection:"column", height:"100%", padding:"48px 36px" }}>
            ...
            <div style={{ marginBottom:36 }} className="fu">
              ...
              <h1 style={{ fontSize:"clamp(22px,2.8vw,30px)", fontWeight:800, color:"#fff", lineHeight:1.2, letterSpacing:"-0.03em", marginBottom:12 }}>
                One login.<br />Every NuGens product.
              </h1>
              <p style={{ fontSize:13.5, color:"rgba(255,255,255,0.4)", lineHeight:1.72, maxWidth:280 }}>
                Sign in once — access Gen-E, HyperX, DigiHub and The Wedding Unit without logging in again.
              </p>
            </div>
            ...
            <div style={{ background:"#fff", border:"1.5px solid #f0f0f0", borderRadius:16, padding:"28px 28px 24px", boxShadow:"0 4px 32px rgba(0,0,0,0.05)" }}>
              <div style={{ display:"flex", background:"#f3f4f6", borderRadius:10, padding:3, marginBottom:22 }}>
                {[["login","Sign In"],["signup","Create Account"]].map(([t,l]) => (
                  <button key={t} className="at" onClick={() => { setTab(t); setError(""); setSuccess(""); }}
                    style={{ background:tab===t?"#fff":"transparent", boxShadow:tab===t?"0 1px 6px rgba(0,0,0,0.08)":"none", color:tab===t?"#0a0a0a":"#9ca3af" }}>{l}</button>
                ))}
              </div>
