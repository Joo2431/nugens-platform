...
// Re-spaced now that Gen-E's own standalone Pro is ₹299/mo: Premium
// (Gen-E unlimited + 5 HyperX courses/mo) and Pro (Gen-E + full HyperX
// access) both sit meaningfully above what Gen-E costs alone, so buying
// Gen-E alone is never nearly the same price as the bundle for strictly
// less value. Pro is set exactly ₹1,000/yr above Premium, per direct
// pricing instruction — still comfortably above Gen-E's own ₹299/mo.
const BASE_PRICES = {
  individual: { starter: 0,   premium: 449,  pro: 549  },   // monthly
  business:   { starter: 499, premium: 999, pro: 1999 },
};
const YEARLY_PRICES = {
  individual: { starter: 0,    premium: 4490,  pro: 5490  },
...
function formatPrice(inrAmount, currency, isYearly) {
  // BUG FIX: this used to multiply by 10 again whenever isYearly was true —
  // but the caller already picks the correct source table (YEARLY_PRICES vs
  // BASE_PRICES) before calling this, so the amount passed in for yearly
  // plans was ALREADY the real yearly price. Multiplying it by 10 again
  // displayed e.g. ₹44,900/yr instead of the intended ₹4,490/yr — a 10x
  // display bug. formatPrice now only applies currency conversion.
  const amount = inrAmount * currency.multiplier;
