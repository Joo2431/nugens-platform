...
const REASONS = [
  ...
  { dot: "#d97706", label: "The Units",          desc: "Productions, bookings, or event enquiries" },
...
export default function Contact() {
  ...
  // Pre-fill reason from ?reason= query param (e.g. from the "Book a discovery call" CTA)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const reason = params.get("reason");
    const validReasons = ["gene", "hyperx", "digihub-brand", "digihub-career", "units", "partnership", "general"];
    if (reason && validReasons.includes(reason)) {
      setForm(f => ({ ...f, reason }));
    }
    if (params.get("intent") === "discovery") setIsDiscoveryCall(true);
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err?.error || "Couldn't send your message. Please try again.");
      }
      setStatus("success");
      setForm(INITIAL);
    } catch (e) {
        ...
        <div className="ct-layout" style={{
          ...
          <div style={{
            ...
            <div style={{ padding: "26px 30px 22px", borderBottom: `1.5px solid ${BORDER}` }}>
              ...
              }}>{isDiscoveryCall ? "Book your discovery call" : "Send us a message"}</h2>
              <p style={{ fontSize: 13.5, color: "#a1a1aa", fontFamily: "'DM Sans', sans-serif" }}>
                {isDiscoveryCall
                  ? "Tell us about your brand and goals — we'll schedule a call to map out how Nugens can help."
              ...
              <form onSubmit={handleSubmit} noValidate style={{ padding: "26px 30px 30px" }}>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  >
                    ...
                    <option value="units">The Units — productions, bookings & packages</option>
                ...
                <div style={{ marginBottom: 22 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
                    {errors.message ? <p className="ct-err" style={{ margin: 0 }}>{errors.message}</p> : <span />}
                    <span style={{
                      fontSize: 11.5,
                      color: form.message.length >= 500 ? "#be123c" : "#6b7280",
