...
function FAQItem({ q, a, isOpen, onToggle, highlight }) {
  return (
    <div style={{ borderBottom:`1px solid ${B}`, overflow:"hidden" }}>
      <button onClick={onToggle} style={{
        ...
        <span style={{ fontSize:14, fontWeight:600, color: highlight ? PINK : "#0a0a0a", paddingRight:16 }}>{q}</span>
...
export default function Support() {
  ...
  const searchResults = query.trim().length > 0
    ? FAQS.flatMap((cat, catIndex) =>
        cat.items
          .filter(item =>
            item.q.toLowerCase().includes(query.trim().toLowerCase()) ||
            item.a.toLowerCase().includes(query.trim().toLowerCase()))
          .map(item => ({ ...item, catIndex, catName: cat.cat }))
      ).slice(0, 8)
    : [];

  const goToResult = (result) => {
    setActiveCat(result.catIndex);
    setOpenItem(result.q);
    ...
    <>
      ...
      <section style={{ padding:"72px 24px 56px", textAlign:"center", borderBottom:`1px solid ${B}` }}>
        <Reveal>
          ...
          {/* Search bar */}
          <div ref={searchRef} style={{ maxWidth:480, margin:"0 auto", position:"relative" }}>
            <input
              value={query}
              placeholder="Search for help…"
              onChange={e => { setQuery(e.target.value); setShowResults(true); }}
              onFocus={e => { e.target.style.borderColor = PINK; setShowResults(true); }}
              onBlur={e => { e.target.style.borderColor = B; }}
              onKeyDown={e => { if (e.key === "Enter" && searchResults[0]) goToResult(searchResults[0]); }}
              style={{
            ...
            {showResults && query.trim().length > 0 && (
              <div style={{
                ...
                boxShadow:"0 8px 24px rgba(0,0,0,0.08)", textAlign:"left", overflow:"hidden",
              }}>
                {searchResults.length > 0 ? searchResults.map(r => (
                  <button key={r.q} onClick={() => goToResult(r)} style={{
                    width:"100%", textAlign:"left", padding:"12px 16px", background:"none",
                    border:"none", borderBottom:`1px solid ${B}`, cursor:"pointer",
                    fontFamily:"'Plus Jakarta Sans',sans-serif",
                  }}>
                    <div style={{ fontSize:13.5, fontWeight:600, color:"#0a0a0a" }}>{r.q}</div>
                    <div style={{ fontSize:11.5, color:"#9ca3af", marginTop:2 }}>{r.catName}</div>
                  </button>
                )) : (
                  <div style={{ padding:"16px", fontSize:13, color:"#9ca3af", textAlign:"center" }}>
                    No results for "{query}" — try the Submit a ticket option below.
                  </div>
                )}
              </div>
            )}
      ...
      {/* FAQ */}
      <section id="faq-section" style={{ padding:"56px 24px 64px", background:"#fafafa", borderBottom:`1px solid ${B}` }}>
            ...
            <div style={{ background:"#fff", borderRadius:14, border:`1px solid ${B}`, padding:"4px 24px" }}>
              {FAQS[activeCat < FAQS.length ? activeCat : 0].items.map(item => (
                <FAQItem key={item.q} q={item.q} a={item.a}
                  ...
                  highlight={openItem === item.q}
                  onToggle={() => setOpenItem(o => o === item.q ? "" : item.q)} />
              ))}
            </div>
          </Reveal>
        </div>
