...
// Fallback only — real categories now come from the hx_categories table via
// /api/hyperx/categories, managed in the Categories tab below. This list is
// used only if that fetch hasn't resolved yet or fails.
const FALLBACK_CATS = [
...
// Per-course pricing removed — HyperX is subscription-only (individual/business
// HyperX plans or the overall Nugens package). Courses are gated by is_free
// (available on the free tier) and is_exclusive (subscribers only), not price.
const EMPTY_COURSE = {
  title:"", description:"", category:"Communication", course_type:"individual",
  level:"Beginner", is_free:false, is_published:false,
...
function getCloudflareStreamId(url) {
  if (!url) return null;
  ...
  const match = value.match(/(?:cloudflarestream\.com|videodelivery\.net)\/([a-f0-9]{32})/i);
  return match?.[1] || null;
}

function normalizeLessonVideoUrl(url) {
  const raw = String(url || "").trim();
  const cloudflareId = getCloudflareStreamId(raw);
  return cloudflareId ? `https://iframe.videodelivery.net/${cloudflareId}` : raw;
}
...
export default function AdminPanel({ profile: profileProp }) {
  ...
  const [editingLessonId, setEditingLessonId] = useState(null); // lesson.id currently being edited, or null
  const thumbRef = useRef();
  const videoRef = useRef();

  const catNames = categories.length ? categories.map(c => c.name) : FALLBACK_CATS;
  ...
  useEffect(() => { if (isAdmin && checked) { loadCourses(); loadCategories(); } }, [isAdmin, checked]);

  const loadCategories = async () => {
    try {
      const { categories: cats } = await apiGet("/api/hyperx/categories");
      setCategories(cats || []);
    } catch (e) {
      console.warn("Failed to load categories, using fallback:", e.message);
    }
  };
  ...
  const addCategory = async () => {
    ...
    setCatSaving(true);
    try {
      await apiPost("/api/hyperx/categories", { name: newCatName.trim() });
      setNewCatName("");
      await loadCategories();
      notify("Category added ✓");
    } catch (e) {
      notify(e.message || "Couldn't add category", "error");
    } finally {
      setCatSaving(false);
    }
  };
  ...
  const removeCategory = async (cat) => {
    ...
    if (inUse > 0 && !window.confirm(`${inUse} course(s) use "${cat.name}". Delete the category anyway? (Those courses will keep the old category text, just unselectable for new courses.)`)) return;
    try {
      await apiDelete(`/api/hyperx/categories/${cat.id}`);
      await loadCategories();
      notify("Category removed");
    } catch (e) {
      notify(e.message || "Couldn't remove category", "error");
    }
  };
  ...
  const saveCourse = async () => {
    ...
    delete payload.price; delete payload.offer_percent; // per-course pricing removed — subscription-only access
  ...
  const saveLesson = async () => {
    ...
    if (editingLessonId) {
      // Update existing lesson via the backend PATCH endpoint (admin-gated)
      try {
        await apiPatch(`/api/hyperx/lessons/${editingLessonId}`, {
          title: lf.title, description: lf.description, section: lf.section,
          duration_mins: Number(lf.duration_mins || 0), sort_order: Number(lf.sort_order || 0),
          is_free: !!lf.is_free, video_url: lf.video_url,
        });
        ...
        notify(e.message || "Couldn't update lesson", "error");
      }
      return;
    }
  ...
  const startEditLesson = (lesson) => {
    ...
    setLf({
      title: lesson.title || "", description: lesson.description || "", section: lesson.section || "",
      duration_mins: lesson.duration_mins || 0, sort_order: lesson.sort_order || 0,
      is_free: !!lesson.is_free, video_url: lesson.video_url || "",
    });
  };
  ...
  if (!checked) return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", height:"100vh", background:LIGHT, flexDirection:"column", gap:12, fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
      ...
      <style>{`@keyframes spin{from{transform:rotate(0)}to{transform:rotate(360deg)}} .hx-ap-courses{display:grid;grid-template-columns:320px 1fr;gap:22px} .hx-ap-lessons{display:grid;grid-template-columns:280px 1fr;gap:22px} @media(max-width:800px){.hx-ap-courses,.hx-ap-lessons{grid-template-columns:1fr!important}}`}</style>
    </div>
  );
    ...
    <div style={S.page}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
        ...
        <div style={{ display:"flex", gap:6, background:CARD, border:`1px solid ${BORDER}`, borderRadius:11, padding:4 }}>
          {["courses","lessons","categories","analytics"].map(t => (
              ...
              {filteredCourses.length === 0 ? (
                ...
                <div key={c.id}
                  ...
                  <div style={{ fontSize:10, color:MUTED, marginTop:3 }}>
                    {c.course_type} · {c.level} · {c.is_free ? "Free tier" : c.is_exclusive ? "Exclusive" : "Subscription"} · {c.total_lessons} lessons
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={S.card}>
            ...
            <div style={S.row3}>
              <div>
                ...
                <select style={S.input} value={cf.category || catNames[0]}
                  onChange={e => setCf(c => ({ ...c, category: e.target.value }))}>
                  {catNames.map(cat => <option key={cat}>{cat}</option>)}
                </select>
              </div>
              <div>
            ...
            {/* Per-course pricing removed — HyperX is subscription-only.
                Access is controlled by the checkboxes below (Free / Exclusive)
                combined with the user's HyperX or Nugens subscription plan. */}
              ...
              <div style={{ marginTop:28, borderTop:`1px solid ${BORDER}`, paddingTop:24 }}>
                ...
                {lessons.slice(0, 5).map((l, i) => (
                  <div key={l.id} className="lesson-row">
                    ...
                    <button onClick={() => { setTab("lessons"); startEditLesson(l); }}
                      style={{ background:"none", border:"none", cursor:"pointer", color:MUTED, fontSize:11, flexShrink:0, marginRight:4 }}>Edit</button>
          ...
          <div style={S.card}>
            {!selCourse ? (
              ...
              <>
                ...
                {lessons.map((l, i) => (
                  <div key={l.id} className="lesson-row">
                    ...
                    <button onClick={() => startEditLesson(l)}
                      style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:7, cursor:"pointer", color:TEXT, fontSize:12, padding:"4px 10px", marginRight:6 }}>
                      Edit
                    </button>
                    <button onClick={() => deleteLesson(l.id)}
                ...
                {/* Add lesson form */}
                <div style={{ background:"#fafafa", border:`1px dashed ${editingLessonId ? PINK : BORDER}`, borderRadius:12, padding:20, marginTop:16 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:TEXT, marginBottom:16 }}>
                    {editingLessonId ? "✎ Editing Lesson" : "Add New Lesson"}
                  </div>
                  ...
                  <div style={{ marginBottom:14 }}>
                    ...
                    <input style={S.input} value={lf.video_url || ""} placeholder="Paste Cloudflare Stream iframe/video link, HLS URL, video ID, or upload below"
                      onChange={e => setLf(l => ({ ...l, video_url: normalizeLessonVideoUrl(e.target.value) }))} />
                  </div>
                  ...
                  <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
                    ...
                    {editingLessonId && (
                      <button style={{ ...S.btnSm, background:CARD, color:MUTED, border:`1px solid ${BORDER}`, marginLeft:"auto" }} onClick={cancelEditLesson}>
                        Cancel
                      </button>
                    )}
                    <button style={{ ...S.btn, marginLeft: editingLessonId ? 0 : "auto" }} onClick={saveLesson}>
      ...
      {tab === "categories" && (
        <div style={{ maxWidth:640 }}>
          <div style={S.card}>
            <div style={{ fontSize:14, fontWeight:800, color:TEXT, marginBottom:6 }}>Course Categories</div>
            <div style={{ fontSize:12, color:MUTED, marginBottom:20 }}>
              Categories drive the course dropdown above, and how students browse courses on the Dashboard. Add or remove categories here — no redeploy needed.
            </div>

            <div style={{ display:"flex", gap:8, marginBottom:20 }}>
              <input style={{ ...S.input, flex:1 }} value={newCatName} placeholder="e.g. Negotiation Skills"
                onChange={e => setNewCatName(e.target.value)}
                onKeyDown={e => { if (e.key === "Enter") addCategory(); }} />
              <button style={S.btn} onClick={addCategory} disabled={catSaving}>
                {catSaving ? "Adding…" : "+ Add Category"}
              </button>
            </div>

            {categories.length === 0 ? (
              <div style={{ fontSize:12, color:MUTED, textAlign:"center", padding:20 }}>
                No categories loaded yet — using the built-in fallback list for course creation until this loads.
              </div>
            ) : (
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {categories.map(cat => {
                  ...
                  return (
                    <div key={cat.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 14px", background:"#f8f9fb", border:`1px solid ${BORDER}`, borderRadius:9 }}>
                      <div>
                        <span style={{ fontSize:13, fontWeight:600, color:TEXT }}>{cat.name}</span>
                        <span style={{ fontSize:11, color:MUTED, marginLeft:10 }}>{courseCount} course{courseCount===1?"":"s"}</span>
                      </div>
                      <button onClick={() => removeCategory(cat)}
                        style={{ background:"#fee2e2", border:"none", borderRadius:7, cursor:"pointer", color:RED, fontSize:11, padding:"4px 10px" }}>
                        Remove
                      </button>
                    </div>
                  );
            ...
            <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
              <thead>
                <tr style={{ background:"#f8f9fb", borderBottom:`2px solid ${BORDER}` }}>
                  {["Title","Type","Level","Access","Lessons","Status"].map(h => (
              ...
              <tbody>
                {courses.map(c => (
                  <tr key={c.id} style={{ borderBottom:`1px solid ${BORDER}`, cursor:"pointer" }}
                    ...
                    <td style={{ padding:"10px 12px", color:c.is_free?GREEN:TEXT }}>{c.is_free ? "Free tier" : c.is_exclusive ? "Exclusive" : "Subscription"}</td>
