...
// No useProfile() hook here — App.jsx already resolved auth and passes it down.
export default function Sidebar({ profile }) {
  ...
  // FIX 2: The previous code called onClick={onSignOut} which was never passed
  // as a prop from App.jsx, causing a runtime error. The signOut function was
  // already defined right here in the component — just use it directly.
  const signOut = async () => {
    ...
    <>
      ...
      >
        ...
        {!collapsed && (
          <div
            style={{
              background:   isBiz ? "#fef2f2" : "#eff6ff",
              border:       `1px solid ${isBiz ? PINK + "30" : "#bfdbfe"}`,
              borderRadius: 9,
              padding:      "7px 11px",
              marginBottom: 14,
            }}
        ...
        <nav style={{ display: "flex", flexDirection: "column", gap: 2, marginBottom: 8 }}>
          {NAV.map((n) => (
            <NavLink
              ...
              className={({ isActive }) => `hx-nav${isActive ? " active" : ""}`}
            >
        ...
        {/* ── DIVIDER ── */}
        {!collapsed && (
          <div style={{ borderTop: `1px solid ${BORDER}`, margin: "8px 4px 10px" }} />
        )}
        ...
        {/* ── SIGN OUT ── */}
        {/* FIX 2: Previously called onClick={onSignOut} — a prop that was never
            passed from App.jsx. The signOut function is defined above in this
            component, so we call it directly here. */}
        {!collapsed && (
          <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 12, marginTop: 4 }}>
            <button className="hx-signout-btn" onClick={signOut}>
              ← Sign out
            </button>
          </div>
        )}
