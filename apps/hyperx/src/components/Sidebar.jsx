...
export default function Sidebar({ profile }) {
  ...
  // Admin if plan=admin OR email in hardcoded list
  ...
  return (
    <>
      <style>{`
        ...
        .hx-nav .hx-ico { color:#d1d5db; font-size:14px; flex-shrink:0; width:18px; text-align:center; }
        .hx-nav:hover .hx-ico, .hx-nav.active .hx-ico { color:${PINK}; }
        .hx-signout { width:100%; padding:9px 13px; background:none; border:1px solid ${BORDER};
        ...
        .hx-signout:hover { border-color:${PINK}40; color:${PINK}; }
      `}</style>
      ...
      <div style={{
        ...
        {!collapsed && (
          <div style={{ display:"flex", alignItems:"center", gap:9, padding:"8px 10px", background:"#f8f9fb", borderRadius:9, marginBottom:16 }}>
            ...
            <div style={{ overflow:"hidden" }}>
              ...
              <div style={{ fontSize:10, color:plan==="free"?MUTED:PINK, fontWeight:600 }}>{planLabel}</div>
            </div>
          </div>
        )}
        ...
        <nav style={{ flex:1, display:"flex", flexDirection:"column", gap:2 }}>
          ...
          {/* Community link */}
          <NavLink to="/community" className={({isActive})=>`hx-nav${isActive?" active":""}`}>
            <span className="hx-ico">◉</span>
            {!collapsed && "Community"}
          </NavLink>
        </nav>
